...
uint64_t send_to_server( struct about_Server pack ) {
...
void *ThreadSend(void *args) {
    struct about_Server *pack = (struct about_Server *)args;
...
int main(int argc, char **argv) {
    ...
    struct about_Server *pack = malloc(sizeof(struct about_Server) * servers_num);
    
    
    
    //потоки для результатов
